...
_ecosystem: {
	...
	"homelab": {
		...
		status:      "degraded" // node-1.fast dead
	}
	"removed": {
		name:        "removed"
	...
	"static-api": {
		...
		depends_on:  {"quicue-patterns": true, homelab: true}
		status:      "active"
	...
	"forgejo": {
		...
		description: "Self-hosted git on node-3 (CT 637)"
		depends_on:  {homelab: true}
		status:      "active"
	}
	"gitlab": {
		name:        "gitlab"
		"@type":     {Service: true, GitServer: true}
		description: "GitLab on node-1 (CT 614) — DEAD: node-1.fast pool gone"
		depends_on:  {homelab: true}
		status:      "dead"
	}
