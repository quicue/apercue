...
d008: core.#Decision & {
	id:        "ADR-008"
	title:     "Public/private site split — operational data stays on homelab"
	status:    "accepted"
	date:      "2026-02-20"
	...
	decision:  "Split build-site.sh into public (landing, spec, vocab, examples) and private (charter, ecosystem, projections) targets. CI deploys only public content to Cloudflare Pages. Private data is served from homelab network via deploy-local.sh."
	...
	consequences: [
		...
		"deploy-local.sh handles full build + optional rsync to homelab target",
